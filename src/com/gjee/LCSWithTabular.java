...
public class LCSWithTabular {
	...
	public static int longestCommonSubsequence(String text1, String text2) {
		int[][] LCSMem = new int[text1.length()+1][];
		for(int i=0;i<text1.length()+1;i++) {
			LCSMem[i]= new int[text2.length()+1];
		}
		...
		return LCSTabular(text1,text2,LCSMem) ;
	}

	public static int LCSTabular(String text1, String text2,int[][] LCSMem) {
			...
			for (int j = 0; j <  text2.length(); j++) {
				...
				if(text1.charAt(i) == text2.charAt(j)) {
					 LCSMem[i+1][j+1] = 1+ LCSMem[i][j];
					  ...
					  LCSMem[i+1][j+1] = max(LCSMem[i][j+1],LCSMem[i+1][j]);
				}
				
			}
		}

		 return LCSMem[text1.length()][text2.length()];
	}
